...
class SegmentTree(object):
    ...
    a) setting item's value is slightly slower. It is O(lg capacity) instead of
       O(1).
    ...
    def __init__(self, capacity, operation, neutral_element):
        ...
        operation : lambda obj, obj -> obj
            and operation for combining elements (eg. sum, max) must form a
            mathematical group together with the set of possible values for
            array elements (i.e. be associative)
        neutral_element : obj
            neutral element for the operation above. eg. float('-inf') for max
            and 0 for sum.
        """
    ...
    def reduce(self, start=0, end=None):
        """Run `self.operation` to a contiguous subsequence of the array.
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def sample(self, batch_size):
        ...
        -------
        weights : np.array
            Array of shape (batch_size,) and dtype np.float32 denoting
            importance weight of each sampled transition
        idxes : np.array
            Array of shape (batch_size,) and dtype np.int32 indices in buffer
    ...
    def update_priorities(self, idxes, priorities):
        ...
        Sets priority of transition at index idxes[i] in buffer to
        priorities[i].
